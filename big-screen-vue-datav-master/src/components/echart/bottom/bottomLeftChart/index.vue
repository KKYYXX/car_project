...
export default {
  ...
  async mounted () {
    ...
    // initialize chart immediately after data is set to ensure render
    // this.setData();
    this.initChart()
  },
  updated(){
  ...
  methods: {
    ...
    initChart(){
      ...
      // compute max for right y axis from lineData to ensure visible scale
      const lineMax = (Array.isArray(this.cdata.lineData) && this.cdata.lineData.length) ? Math.max.apply(null, this.cdata.lineData) : 1;
      // avoid zero max
      const rightMax = lineMax > 0 ? lineMax * 1.2 : 1;
      console.log('lineData', this.cdata.lineData, 'rightMax', rightMax)
      const option={
          tooltip: {
            trigger: "axis",
          ...
          // set right axis max based on data
          // will be applied below in yAxis[1]
          series: [
            {
              name: "价格（万元）",
              type: "line",
      ...
      // apply computed right axis max
      if (option && option.yAxis && option.yAxis[1]) {
